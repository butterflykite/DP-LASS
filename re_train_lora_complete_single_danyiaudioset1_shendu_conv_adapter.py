...
from shendu_adapter.conv_adapter import ConvAdapterManager
...
class ModelSaveCallback(Callback):
    ...
    def __init__(self, save_dir, model_name_prefix="conv_adapter_model", start_epoch=0):
        super().__init__()
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        ...
        if trainer.current_epoch >= self.start_epoch:
            ...
            # 保存模型（只保存DoRA模型的状态字典）
            torch.save({
                'dora_state_dict': pl_module.conv_adapter.conv_adapter_model.state_dict(),
...
class DoRAAudioSep(pl.LightningModule):
    ...
    def __init__(
        ...
        self.batch_size=batch_size
    def forward(self, x):
        return self.conv_adapter.forward(x)
    ...
    def training_step(self, batch_data_dict, batch_idx):
        ...
        # 确保conv_adapter在正确的设备上
        if hasattr(self.conv_adapter, 'conv_adapter_model') and next(self.conv_adapter.conv_adapter_model.parameters()).device != device:
            self.conv_adapter.conv_adapter_model = self.conv_adapter.conv_adapter_model.to(device)
    ...
    def validation_step(self, batch_data_dict, batch_idx):
        ...
        with torch.no_grad():
            ...
            output_dict = self.conv_adapter.conv_adapter_model(input_dict)
    ...
    def configure_optimizers(self):
        ...
        optimizer = optim.AdamW(
            [p for p in self.conv_adapter.conv_adapter_model.parameters() if p.requires_grad],
...
def train_dora(args):
    ...
    json_file = os.path.join( f"/s1home/zyk524/AudioSep/LORA/s4/cluster0.json")
    ...
    val_dataset = CSVAudioTextDatasetForEval(
        csv_file=f"/s1home/zyk524/AudioSep/LORA/s4/csv/cluster0.csv",
        sampling_rate=sampling_rate,
    )
    ...
    conv_adapter = ConvAdapterManager(base_model)
    k = sum(p.numel() for p in conv_adapter.conv_adapter_model.parameters() if p.requires_grad)  # 计算可训练参数总数
    print('# conv_adapter_model of parameters:', k)  # 打印参数数量
    ...
    torch.save({
        'dora_state_dict': conv_adapter.conv_adapter_model.state_dict(),
    }, final_save_path)
...
def main():
    ...
    parser.add_argument(
        "--workspace",
        type=str,
        default="/s1home/zyk524/AudioSep/LORA/shendu_adapter/cluster0",
        help="工作目录路径"
    )
    parser.add_argument(
        "--config_yaml",
