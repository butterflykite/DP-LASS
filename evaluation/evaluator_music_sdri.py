...
class SmartDoRAEvaluator:
    ...
    def __init__(
            self,
            metadata_csv='evaluation/metadata/music_eval.csv',
            audio_dir='evaluation/data/music',
            sampling_rate=32000,
            query='caption',
    ) -> None:
        """初始化最佳DoRA评估器
    ...
    def __call__(
        ...
        print(f"\n===== 开始样本评估 =====")
        all_results = []
        performance = {"sdri": [], "sisdr": []}
        source_type_performance = {source_type: {"sdri": [], "sisdr": []} for source_type in self.source_types}
        ...
        for eval_data in tqdm(self.eval_list, desc="评估样本"):
            ...
            performance["sdri"].append(result["sdri"])
            performance["sisdr"].append(result["sisdr"])
        
            
            # 按音乐类别分组记录结果
            caption = result["caption"]
            if caption in self.source_types:
                source_type_performance[caption]["sdri"].append(result["sdri"])
                source_type_performance[caption]["sisdr"].append(result["sisdr"])
              

        avg_performance = {
            "avg_sdri": np.mean(performance["sdri"]) if performance["sdri"] else 0,
            "avg_sisdr": np.mean(performance["sisdr"]) if performance["sisdr"] else 0,
        }
        
        # 计算每个音乐类别的平均性能
        source_type_avg_performance = {}
        for source_type in self.source_types:
            if source_type_performance[source_type]["sdri"]:
                source_type_avg_performance[source_type] = {
                    "avg_sdri": np.mean(source_type_performance[source_type]["sdri"]),
                    "avg_sisdr": np.mean(source_type_performance[source_type]["sisdr"])
                }
        
        # 计算所有音乐类别平均值的平均值
        mean_sdri_list = []
        mean_sisdr_list = []
        mean_total_score_list = []
        
        for source_type, perf in source_type_avg_performance.items():
            mean_sdri_list.append(perf["avg_sdri"])
            mean_sisdr_list.append(perf["avg_sisdr"])
        
        source_type_mean_performance = {
            "mean_avg_sdri": np.mean(mean_sdri_list) if mean_sdri_list else 0,
            "mean_avg_sisdr": np.mean(mean_sisdr_list) if mean_sisdr_list else 0,
        }
        return {
            "source_type_mean_performance": source_type_mean_performance
        }

def main():
    """主函数，用于测试最佳DoRA评估器（无参考音频版本）"""
    parser = argparse.ArgumentParser(description='最佳DoRA模型评估器')
    parser.add_argument('--metadata_csv', type=str, default='/home/zyk524/test/audio/evaluation/metadata/music_eval.csv',
                        help='测试数据的元数据CSV文件路径')
    parser.add_argument('--audio_dir', type=str, default='/dataset/AudioSep_eval/music',
                        help='评估音频段的目录')
    ...
    print(f"整体平均性能:  sdri={results['source_type_mean_performance']['mean_avg_sdri']:.3f}, sisdr={results['source_type_mean_performance']['mean_avg_sisdr']:.3f}")
