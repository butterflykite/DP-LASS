...
class SmartDoRAEvaluator:
    ...
    def evaluate_sample(self, eval_data):
        ...
        """
        idx, file_id, mix_wav, s0_wav, s0_text,s1_wav,s1_text= eval_data
        labels = s0_text
        # 构建混合音频的路径（不再需要参考音频）
...
def main():
    ...
    parser.add_argument('--metadata_csv', type=str, default='/s7home/zyk524/test/audio/evaluation/metadata/vggidx_eval.csv',
                        help='测试数据的元数据CSV文件路径')
    parser.add_argument('--audio_dir', type=str, default='/dataset/AudioSep_eval/vggsound',
                        help='评估音频段的目录')
