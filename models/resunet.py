...
class FiLM(nn.Module):
    def __init__(self, film_meta, condition_size,num_filters):
        ...
        # )
        # 投影层：生成M倍的条件参数
        self.projection = nn.Linear(condition_size, condition_size * num_filters)
        # 为每个滤波器创建独立的FiLM模块
        self.film_layers = nn.ModuleList([
            self.create_film_layers(film_meta) for _ in range(num_filters)
        ])
    def _create_film_layers(self, film_meta):
        modules, _ = self.create_film_modules(film_meta, ancestor_names=[])
    ...
    def forward(self, conditions):
        ...
        #
        # return film_dict
        projected = self.projection(conditions)  # [batch_size, condition_size * M]
        projected = projected.view(-1, self.num_filters, self.condition_size)
        film_dicts = []
        for m in range(self.num_filters):
            film_dict = self.calculate_film_data(projected[:, m], self.film_layers[m])
            film_dicts.append(film_dict)
        return film_dicts
        # film_dicts = []
        # for m in range(self.num_filters):
        #     # 独立条件投影
        #     cond_m = self.projections[m](conditions)
        #     # 生成第m个滤波器的FiLM参数
        #     film_dict = self.calculate_film_data(cond_m, self.film_layers[m])
        #     film_dicts.append(film_dict)
        # return film_dicts  # 返回多个film_dict
...
class ResUNet30_Base(nn.Module, Base):
    def __init__(self, input_channels, output_channels, num_filters=3):
        ...
        #     bias=True,
        # )
        self.after_convs = nn.ModuleList([
            nn.Conv2d(
                in_channels=32,
                out_channels=output_channels * self.K,
                kernel_size=(1, 1),
                ...
                padding=(0, 0),
                bias=True,
            ) for _ in range(num_filters)  # 创建M个输出层
        ])
        self.init_weights()
 ...
 # 多滤波器解码
        outputs = []
        for m in range(self.num_filters):
            x_m = self.after_convs[m](x12)  # 每个滤波器使用独立的输出层
            # Recover shape等后续操作
            x_m = F.pad(x_m, pad=(0, 1))
            x_m = x_m[:, :, 0:origin_len, :]
            # 生成波形
            separated_audio = self.feature_maps_to_wav(
                input_tensor=x_m,
                sp=mag,
            ...
            outputs.append(separated_audio)
        return outputs  # 返回多个输出
