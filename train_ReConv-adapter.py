...
from loramodel.lora_simple_single32 import DoRAManager
...
def train_dora(args):
    ...
    json_file = os.path.join( f"/home/zyk524/MSCLAP/audioset_eval/cluster7/cluster2.json")
    ...
    val_dataset = CSVAudioTextDatasetForEval(
        csv_file=f"/home/zyk524/MSCLAP/audioset_eval/cluster7csv/cluster2.csv",
        sampling_rate=sampling_rate,
    )
    ...
    # 创建TensorBoard日志记录器
    logger = TensorBoardLogger(save_dir=tf_logs_dir, name=f"cluster6")
    ...
    callbacks = [
        ...
        ModelCheckpoint(
            ...
            filename=f"dora2_last",
            save_top_k=1,
            save_last=True,
        ),
        # 学习率监控
        LearningRateMonitor(logging_interval='step'),
        # 在达到50个epoch后每个epoch保存一次模型
        ModelSaveCallback(
            save_dir=checkpoints_dir,
            model_name_prefix="dora6_model",
            start_epoch=0
        ),
    ]
    # 创建训练器
    ...
    # 保存最终模型
    final_save_path = os.path.join(checkpoints_dir, f"dora_cluster5_final.pt")
    torch.save({
...
def main():
    ...
    parser.add_argument(
        "--workspace",
        type=str,
        default="/home/zyk524/MSCLAP/audioset_eval/testtrain/cluster2/lora32/clip2/re_0.0004",
        help="工作目录路径"
    )
    parser.add_argument(
        "--config_yaml",
        type=str,
        default="/home/zyk524/test/audio/config/audiosep_base.yaml",
        help="配置文件路径"
    )
    parser.add_argument(
        "--checkpoint_path",
        type=str,
        default="/home/zyk524/test/audio/checkpoint/audiosep_base_4M_steps.ckpt",
        help="预训练模型检查点路径"
    )
