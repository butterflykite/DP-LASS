...
from loraandconvlora.conv_lora import LoRAManager
...
class ModelSaveCallback(Callback):
    ...
    def __init__(self, save_dir, model_name_prefix="lora_model", start_epoch=0):
        super().__init__()
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        ...
        if trainer.current_epoch >= self.start_epoch:
            ...
            # 保存模型（只保存DoRA模型的状态字典）
            torch.save({
                'dora_state_dict': pl_module.lora_manager.lora_model.state_dict(),
...
class DoRAAudioSep(pl.LightningModule):
    ...
    def __init__(
        ...
        self.batch_size=batch_size
    def forward(self, x):
        return self.lora_manager.forward(x)
    ...
    def training_step(self, batch_data_dict, batch_idx):
        ...
        # 确保lora_manager在正确的设备上
        if hasattr(self.lora_manager, 'lora_model') and next(self.lora_manager.lora_model.parameters()).device != device:
            self.lora_manager.lora_model = self.lora_manager.lora_model.to(device)
    ...
    def validation_step(self, batch_data_dict, batch_idx):
        ...
        with torch.no_grad():
            ...
            output_dict = self.lora_manager.lora_model(input_dict)
    ...
    def configure_optimizers(self):
        ...
        optimizer = optim.AdamW(
            [p for p in self.lora_manager.lora_model.parameters() if p.requires_grad],
...
def train_dora(args):
    ...
    lora_manager = LoRAManager(base_model)
    k = sum(p.numel() for p in lora_manager.lora_model.parameters() if p.requires_grad)  # 计算可训练参数总数
    print('# lora_model of parameters:', k)  # 打印参数数量
    ...
    torch.save({
        'dora_state_dict': lora_manager.lora_model.state_dict(),
    }, final_save_path)
...
def main():
    ...
    parser.add_argument(
        "--workspace",
        type=str,
        default="/s1home/zyk524/AudioSep/LORA/CONLORA/cluster6",
        help="工作目录路径"
    )
    parser.add_argument(
        "--config_yaml",
